...
if not data_fixer:
    ...
    final_projects_tasks_distinctCnt_df = final_projects_tasks_distinctCnt_df.withColumn("time_stamp", lit(date_format(date_sub(current_timestamp(), 1), "yyyy-MM-dd HH:mm:ss.SSS")))
...
successLogger.debug("Logic for submission distinct count end time  " + str(datetime.datetime.now()))
...
new_row = {
    ...
    "dataSource": ml_distinctCnt_projects_datasource,
    "taskId": distinctCnt_projects_start_supervisor.json()["task"],
    "taskCreatedDate" : str(datetime.datetime.now().date())
}

new_row['statusCode'] = distinctCnt_projects_start_supervisor.status_code
