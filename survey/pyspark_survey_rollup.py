...
final_df.coalesce(1).write.format("json").mode("overwrite").save(
        config.get("OUTPUT_DIR", "survey_rollup")+"/"
        )

for filename in os.listdir(config.get("OUTPUT_DIR", "survey_rollup")+"/"):
   if filename.endswith(".json"):
      os.rename(
         config.get("OUTPUT_DIR", "survey_rollup") + "/" + filename,
         config.get("OUTPUT_DIR", "survey_rollup") + "/sl_survey_rollup.json"
      )
