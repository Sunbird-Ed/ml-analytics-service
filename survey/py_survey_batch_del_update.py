...
querySpec = json.loads(config.get("DRUID","survey_query_spec"))
querySpec["filter"]= {"type":"in","dimension":"surveySubmissionId","values":subId} 
...
file_path = config.get("OUTPUT_DIR","survey_druid_data") + "druidData.json"
...
if responseQuery.status_code == 200 :
    ...
    for druid_data in responseDataArr : 
      ...
      for event_data in druid_data["events"]:          
         ...
         print(cntr,event_data["surveySubmissionId"])
...
survey_spec = json.loads(config.get("DRUID","survey_injestion_spec"))


batch_task = requests.post(druid_batch_end_point, data=json.dumps(survey_spec), headers=header)
datasource_name = survey_spec["spec"]["dataSchema"]["dataSource"]
